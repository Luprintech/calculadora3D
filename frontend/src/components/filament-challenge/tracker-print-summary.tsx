    ...
    <div className="bg-white p-8 font-body text-black" style={{ WebkitPrintColorAdjust: 'exact', printColorAdjust: 'exact' as 'exact' }}>
      ...
      {project.coverImage && (
        <div className="mb-6 flex items-center justify-center overflow-hidden rounded-xl border bg-gray-50 p-3">
          <img src={project.coverImage} alt={t('cover_alt', { title: project.title })} className="max-h-64 w-auto max-w-full object-contain" />
        </div>
      )}
      ...
      <div className="mb-6 grid grid-cols-2 gap-4 text-sm">
        {[
          { label: t('hero_pieces'), value: `${project.totalPieces} / ${project.goal}`, border: '#d966ff', bg: 'linear-gradient(135deg, rgba(217,102,255,0.10), rgba(255,255,255,0.92))' },
          { label: t('hero_time'), value: secsToString(project.totalSecs), border: '#23c7ff', bg: 'linear-gradient(135deg, rgba(35,199,255,0.10), rgba(255,255,255,0.92))' },
          { label: t('hero_filament'), value: `${project.totalGrams.toFixed(1)}g`, border: '#37e3a5', bg: 'linear-gradient(135deg, rgba(55,227,165,0.10), rgba(255,255,255,0.92))' },
          { label: t('tracker_print_filament_cost'), value: formatCost(project.totalCost, project.currency), border: '#ffd166', bg: 'linear-gradient(135deg, rgba(255,209,102,0.12), rgba(255,255,255,0.92))' },
        ].map((item) => (
          <div
            key={item.label}
        ...
        >
          <strong>{t('tracker_print_progress')}:</strong> {progressPct}%
        </div>
      </div>
      ...
      <section style={{ breakBefore: 'page', pageBreakBefore: 'always' }}>
        <h2 className="mb-1 text-lg font-bold">{t('tracker_print_piece_list_title')}</h2>
        <p className="mb-4 text-sm text-gray-600">{t('pieces_count', { count: project.totalPieces })}</p>
        <div className="space-y-3 text-sm">
          {orderedPieces.length === 0 ? (
            <p className="text-gray-600">{t('tracker_print_piece_empty')}</p>
          ) : (
            orderedPieces.map((piece) => (
              ...
              >
                ...
                <div className="grid grid-cols-3 gap-3 text-gray-800">
                  ...
                  >
                    <strong>{t('pieces_time')}:</strong> {secsToString(piece.totalSecs)}
                  </div>
                  <div
                    className="rounded-lg px-3 py-2"
                    style={{ background: 'rgba(55,227,165,0.10)' }}
                  >
                    <strong>{t('pieces_grams')}:</strong> {piece.totalGrams.toFixed(1)}g
                  </div>
                  <div
                    className="rounded-lg px-3 py-2"
                    style={{ background: 'rgba(255,209,102,0.14)' }}
                  >
                    <strong>{t('pieces_cost')}:</strong> {formatCost(piece.totalCost, project.currency)}
                  </div>
                </div>
              </div>
            ))
      ...
      <footer className="mt-12 border-t pt-6 text-center text-xs text-gray-700">
        ...
        <div className="flex items-center justify-center gap-4 text-gray-700">
          <a
            ...
            aria-label={t('social_youtube_label')}
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            aria-label={t('social_tiktok_label')}
            className="transition-colors hover:text-primary"
          >
